...
def check_constraints(time_eval, cu_total_linear, cu_total_nonlinear):
    ...
    for label, idx in zip(labels, control_indices):
        ...
        linear_violations = np.where(np.abs(cu_total_linear[:, idx]) > constraints_max[idx])[0]
        nonlinear_violations = np.where(np.abs(cu_total_nonlinear[:, idx] >= constraints_max[idx]))[0]

        # Prepare the violation results for linear
        if len(linear_violations) > 0:
            linear_result = f"Yes, {time_eval[linear_violations[-1]]}"
        else:
            linear_result = "No"

        # Prepare the violation results for nonlinear
        if len(nonlinear_violations) > 0:
            nonlinear_result = f"Yes, {time_eval[nonlinear_violations[-1]]}"
        else:
            nonlinear_result = "No"
